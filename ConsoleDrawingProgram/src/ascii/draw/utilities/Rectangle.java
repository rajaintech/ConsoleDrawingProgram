...
public class Rectangle implements Shape {
	private final static char DRAWSTRING = 'x';
	...
	public void addTo(Canvas canvas) {
		...
		for (int i = 0; i < width(); i++)
			canvas.lineAt(x1 + i, y1, DRAWSTRING);
		for (int i = 0; i < width(); i++)
			canvas.lineAt(x1 + i, y2, DRAWSTRING);

		for (int i = 0; i < height() - 2; i++)
			canvas.lineAt(x1, y1 + i + 1, DRAWSTRING);
		for (int i = 0; i < height() - 2; i++)
			canvas.lineAt(x2, y1 + i + 1, DRAWSTRING);
	}
